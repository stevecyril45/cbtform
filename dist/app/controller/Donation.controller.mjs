import donationService from "../service/Donation.service.mjs";
...
class DonationController {
  // To Create, request must contain an array of donations[] which must be up to 5% of MAX_RECORDS_PER_FILE
  ...
  async create(req, res) {
    ...
    if (!Array.isArray(donations) || donations.length === 0 || donations.length > (MAX_RECORDS_PER_FILE * 0.05)) {
      return res.status(400).json({ error: 'Invalid input: donations must be a non-empty array with max 50 items' });
    }
    ...
    for (const donation of donations) {
      try {
        const result = await donationService.insertDonation(donation);
        if (result.success) {
          successful.push({ ...donation, id: result.id || donation.id });
        } else {
          failed.push({ ...donation, errors: result.errors || ['Unknown error'] });
        }
      } catch (err) {
        failed.push({ ...donation, errors: [err.message] });
      }
    }

    res.status(successful.length > 0 ? 201 : 207).json({
      message: `Processed ${successful.length} successful inserts out of ${donations.length}`,
      successful,
      failed
    });
  }
  ...
  async read(req, res) {
    try {
      ...
      const folderPath = donationService.readDayFolder(y, m, d);
  ...
  async update(req, res) {
    const { donations } = req.body; // Expect array of { year, month, day, id, data }
    if (!Array.isArray(donations) || donations.length === 0 || donations.length > (MAX_RECORDS_PER_FILE * 0.05)) {
      return res.status(400).json({ error: 'Invalid input: donations must be a non-empty array with max 50 items' });
    }
    ...
    for (const item of donations) {
      ...
      try {
        const result = await donationService.updateDonation(year, month, day, id, data);
        if (result) {
    ...
    res.status(successful.length > 0 ? 200 : 207).json({
      message: `Processed ${successful.length} successful updates out of ${donations.length}`,
      successful,
      failed
    });
  }
  ...
  async delete(req, res) {
    const { donations } = req.body; // Expect array of { year, month, day, id }
    if (!Array.isArray(donations) || donations.length === 0 || donations.length > (MAX_RECORDS_PER_FILE * 0.05)) {
      return res.status(400).json({ error: 'Invalid input: donations must be a non-empty array with max 50 items' });
    }
    ...
    for (const item of donations) {
      ...
      try {
        ...
        const result = await donationService.updateDonation(year, month, day, id, deletedData);
        if (result) {
    ...
    res.status(successful.length > 0 ? 200 : 207).json({
      message: `Processed ${successful.length} successful deletes out of ${donations.length}`,
      successful,
      failed
    });
  }
  ...
  async all(req, res) {
    try {
      ...
      const days = await donationService.getAllDaysInMonth(year, month);
