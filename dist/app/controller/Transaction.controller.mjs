import transactionService from "../service/Transaction.service.mjs";
...
class TransactionController {
  // To Create, request must contain an array of transactions[] which must be up to 5% of MAX_RECORDS_PER_FILE
  ...
  async create(req, res) {
    ...
    if (!Array.isArray(transactions) || transactions.length === 0 || transactions.length > (MAX_RECORDS_PER_FILE * 0.05)) {
      return res.status(400).json({ error: 'Invalid input: transactions must be a non-empty array with max 50 items' });
    }
    ...
    for (const transaction of transactions) {
      try {
        const result = await transactionService.insertTransaction(transaction);
        if (result.success) {
          successful.push({ ...transaction, id: result.id || transaction.id });
        } else {
          failed.push({ ...transaction, errors: result.errors || ['Unknown error'] });
        }
      } catch (err) {
        failed.push({ ...transaction, errors: [err.message] });
      }
    }

    res.status(successful.length > 0 ? 201 : 207).json({
      message: `Processed ${successful.length} successful inserts out of ${transactions.length}`,
      successful,
      failed
    });
  }
  ...
  async read(req, res) {
    try {
      ...
      const folderPath = transactionService.readDayFolder(y, m, d);
  ...
  async update(req, res) {
    const { transactions } = req.body; // Expect array of { year, month, day, id, data }
    if (!Array.isArray(transactions) || transactions.length === 0 || transactions.length > (MAX_RECORDS_PER_FILE * 0.05)) {
      return res.status(400).json({ error: 'Invalid input: transactions must be a non-empty array with max 50 items' });
    }
    ...
    for (const item of transactions) {
      ...
      try {
        const result = await transactionService.updateTransaction(year, month, day, id, data);
        if (result) {
    ...
    res.status(successful.length > 0 ? 200 : 207).json({
      message: `Processed ${successful.length} successful updates out of ${transactions.length}`,
      successful,
      failed
    });
  }
  ...
  async delete(req, res) {
    const { transactions } = req.body; // Expect array of { year, month, day, id }
    if (!Array.isArray(transactions) || transactions.length === 0 || transactions.length > (MAX_RECORDS_PER_FILE * 0.05)) {
      return res.status(400).json({ error: 'Invalid input: transactions must be a non-empty array with max 50 items' });
    }
    ...
    for (const item of transactions) {
      ...
      try {
        ...
        const result = await transactionService.updateTransaction(year, month, day, id, deletedData);
        if (result) {
    ...
    res.status(successful.length > 0 ? 200 : 207).json({
      message: `Processed ${successful.length} successful deletes out of ${transactions.length}`,
      successful,
      failed
    });
  }
  ...
  async all(req, res) {
    try {
      ...
      const days = await transactionService.getAllDaysInMonth(year, month);
