import warehouseService from "../service/Warehouse.service.mjs";
...
class WarehouseController {
  // To Create, request must contain an array of warehouses[] which must be up to 5% of MAX_RECORDS_PER_FILE
  ...
  async create(req, res) {
    ...
    if (!Array.isArray(warehouses) || warehouses.length === 0 || warehouses.length > (MAX_RECORDS_PER_FILE * 0.05)) {
      return res.status(400).json({ error: 'Invalid input: warehouses must be a non-empty array with max 50 items' });
    }
    ...
    for (const warehouse of warehouses) {
      try {
        const result = await warehouseService.insertWarehouse(warehouse);
        if (result.success) {
          successful.push({ ...warehouse, id: result.id || warehouse.id });
        } else {
          failed.push({ ...warehouse, errors: result.errors || ['Unknown error'] });
        }
      } catch (err) {
        failed.push({ ...warehouse, errors: [err.message] });
      }
    }

    res.status(successful.length > 0 ? 201 : 207).json({
      message: `Processed ${successful.length} successful inserts out of ${warehouses.length}`,
      successful,
      failed
    });
  }
  ...
  async read(req, res) {
    try {
      ...
      const folderPath = warehouseService.readDayFolder(y, m, d);
  ...
  async update(req, res) {
    const { warehouses } = req.body; // Expect array of { year, month, day, id, data }
    if (!Array.isArray(warehouses) || warehouses.length === 0 || warehouses.length > (MAX_RECORDS_PER_FILE * 0.05)) {
      return res.status(400).json({ error: 'Invalid input: warehouses must be a non-empty array with max 50 items' });
    }
    ...
    for (const item of warehouses) {
      ...
      try {
        const result = await warehouseService.updateWarehouse(year, month, day, id, data);
        if (result) {
    ...
    res.status(successful.length > 0 ? 200 : 207).json({
      message: `Processed ${successful.length} successful updates out of ${warehouses.length}`,
      successful,
      failed
    });
  }
  ...
  async delete(req, res) {
    const { warehouses } = req.body; // Expect array of { year, month, day, id }
    if (!Array.isArray(warehouses) || warehouses.length === 0 || warehouses.length > (MAX_RECORDS_PER_FILE * 0.05)) {
      return res.status(400).json({ error: 'Invalid input: warehouses must be a non-empty array with max 50 items' });
    }
    ...
    for (const item of warehouses) {
      ...
      try {
        ...
        const result = await warehouseService.updateWarehouse(year, month, day, id, deletedData);
        if (result) {
    ...
    res.status(successful.length > 0 ? 200 : 207).json({
      message: `Processed ${successful.length} successful deletes out of ${warehouses.length}`,
      successful,
      failed
    });
  }
  ...
  async all(req, res) {
    try {
      ...
      const days = await warehouseService.getAllDaysInMonth(year, month);
