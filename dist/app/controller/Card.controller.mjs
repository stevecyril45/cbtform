import cardService from "../service/Card.service.mjs";
...
class CardController {
  // To Create, request must contain an array of cards[] which must be up to 5% of MAX_RECORDS_PER_FILE
  ...
  async create(req, res) {
    ...
    if (!Array.isArray(cards) || cards.length === 0 || cards.length > (MAX_RECORDS_PER_FILE * 0.05)) {
      return res.status(400).json({ error: 'Invalid input: cards must be a non-empty array with max 50 items' });
    }
    ...
    for (const card of cards) {
      try {
        const result = await cardService.insertCard(card);
        if (result.success) {
          successful.push({ ...card, id: result.id || card.id });
        } else {
          failed.push({ ...card, errors: result.errors || ['Unknown error'] });
        }
      } catch (err) {
        failed.push({ ...card, errors: [err.message] });
      }
    }

    res.status(successful.length > 0 ? 201 : 207).json({
      message: `Processed ${successful.length} successful inserts out of ${cards.length}`,
      successful,
      failed
    });
  }
  ...
  async read(req, res) {
    try {
      ...
      const folderPath = cardService.readDayFolder(y, m, d);
  ...
  async update(req, res) {
    const { cards } = req.body; // Expect array of { year, month, day, id, data }
    if (!Array.isArray(cards) || cards.length === 0 || cards.length > (MAX_RECORDS_PER_FILE * 0.05)) {
      return res.status(400).json({ error: 'Invalid input: cards must be a non-empty array with max 50 items' });
    }
    ...
    for (const item of cards) {
      ...
      try {
        const result = await cardService.updateCard(year, month, day, id, data);
        if (result) {
    ...
    res.status(successful.length > 0 ? 200 : 207).json({
      message: `Processed ${successful.length} successful updates out of ${cards.length}`,
      successful,
      failed
    });
  }
  ...
  async delete(req, res) {
    const { cards } = req.body; // Expect array of { year, month, day, id }
    if (!Array.isArray(cards) || cards.length === 0 || cards.length > (MAX_RECORDS_PER_FILE * 0.05)) {
      return res.status(400).json({ error: 'Invalid input: cards must be a non-empty array with max 50 items' });
    }
    ...
    for (const item of cards) {
      ...
      try {
        ...
        const result = await cardService.updateCard(year, month, day, id, deletedData);
        if (result) {
    ...
    res.status(successful.length > 0 ? 200 : 207).json({
      message: `Processed ${successful.length} successful deletes out of ${cards.length}`,
      successful,
      failed
    });
  }
  ...
  async all(req, res) {
    try {
      ...
      console.log('here')
      const days = await cardService.getAllDaysInMonth(year, month);
